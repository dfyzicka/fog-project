...
usage() {
    ...
    echo -e "\t-h -? --help\t\t\tОтображать эту информацию"
    echo -e "\t-o    --oldcopy\t\t\tкопировать старые данные"
    echo -e "\t-d    --no-defaults\t\tDon't guess defaults"
    echo -e "\t-U    --no-upgrade\t\tне обновлять"
    echo -e "\t-H    --no-htmldoc\t\tНет htmldoc, не означает PDF"
    echo -e "\t-S    --force-https\t\tForce HTTPS for all comunication"
    echo -e "\t-C    --recreate-CA\t\tПересоздать ключи CA"
    echo -e "\t-K    --recreate-keys\t\tПересоздать ключи SSL"
    echo -e "\t-Y -y --autoaccept\t\tАвтоматическое всё принять по умолчанию и установить"
    echo -e "\t-f    --file\t\t\tИспользуйте другой файл обновления"
    echo -e "\t-c    --ssl-path\t\tУкажите путь SSL"
    echo -e "\t               \t\t\t\tdefaults to /opt/fog/snapins/ssl"
    echo -e "\t-D    --docroot\t\t\tУкажите Apache Docroot для тумана"
    echo -e "\t               \t\t\t\tdefaults to OS DocumentRoot"
    echo -e "\t-W    --webroot\t\t\tУкажите URL -адрес Web Root Want FOG для использования"
    echo -e "\t            \t\t\t\t(E.G. http://127.0.0.1/fog,"
    echo -e "\t            \t\t\t\t      http://127.0.0.1/)"
    echo -e "\t            \t\t\t\tDefaults to /fog/"
    echo -e "\t-B    --backuppath\t\tУкажите путь резервного копирования"
    echo -e "\t      --uninstall\t\tУдалить туман"
    echo -e "\t-s    --startrange\t\tДиапазон старта DHCP"
    echo -e "\t-e    --endrange\t\tКонечный диапазон DHCP"
    echo -e "\t-b    --bootfile\t\tDHCP загрузочный файл"
    echo -e "\t-E    --no-exportbuild\t\tПропустить строительство файла NFS "
    echo -e "\t-X    --exitFail\t\tНе выходите, если пункт не удается"
    echo -e "\t-T    --no-tftpbuild\t\tНе восстанавливайте файл конфигурации TFTPD"
    echo -e "\t-F    --no-vhost\t\tНе перезаписать файл vhost"
    echo -e "\t-A    --arm-support\t\tУстановите ядро и initrd для платформ ARM"
    exit 0
}
